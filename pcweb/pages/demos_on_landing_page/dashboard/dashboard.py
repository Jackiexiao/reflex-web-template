...
def category_items(categroyName: str, count: str, change: str, clicked: bool):
    return rx.vstack(
        ...
        rx.text(change, line_height="1", font_size="12px", color="#A1A1AA"),
        height="5em",
        width="9em",
...
def recent_sale_item(first_name, last_name, sale_amount):
    ...
    formatted_sale_amount = f"+${sale_amount:,.2f}"
    return rx.table.row(
        rx.table.cell(
            rx.text(
                fullname, color=rx.color("mauve", 12), line_height="1", font_size="12px"
            )
        ),
        rx.table.cell(
            rx.text(
                email[0], color=rx.color("mauve", 12), line_height="1", font_size="12px"
            )
        ),
        rx.table.cell(
